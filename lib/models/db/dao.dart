...
class TasksDao extends DatabaseAccessor<MyDatabase> with _$TasksDaoMixin{
  ...
  Future<void> addTask(TaskRecord taskRecord) =>into(taskRecords).insert(taskRecord);
  //Read
  Future<List<TaskRecord>> get allTasks => select(taskRecords).get();
  //Update
  Future<void> updateTask(TaskRecord taskRecord) => update(taskRecords).replace(taskRecord);
  //Delete
